...
while cap.isOpened():
    ...
    if multiHandDetection:
        ...
        #cv2.imwrite(f'captured_image_{name}.jpg', img)
        #print(f"Image captured and saved as 'captured_image_{name}.jpg'") 

        # Capture and save the active window screenshot
        active_window = win32gui.GetForegroundWindow()
        window_rect = win32gui.GetWindowRect(active_window)
        screenshot = pyautogui.screenshot(region=window_rect)
        screenshot.save(f'captured_active_window_{name}.png')
        print(f"Active window screenshot captured and saved as 'captured_active_window_{name}.png'")
